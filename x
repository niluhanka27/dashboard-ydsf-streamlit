...
with st.sidebar:
    ...
    selected_page = st.radio("Pilih Halaman:", options=["Ringkasan Eksekutif", "Data Penerima Bantuan", "Analisis Eksploratif (EDA)", "Profiling Cluster"])

    selected_program = None
    if selected_page != "Ringkasan Eksekutif":
...
# ==================================================================
#                  1. HALAMAN UTAMA (RINGKASAN EKSEKUTIF)
# ==================================================================
if selected_page == "Ringkasan Eksekutif":
    ...
    if not df_all.empty:
        ...
        with col1:
            ...
            <b>Insight:</b> Program **{program_terbesar}** menyalurkan dana terbesar (Rp {nilai_terbesar:,.0f}), menandakan pilar utama lembaga. Program yang berada di atas garis merah memiliki alokasi di atas rata-rata.
        ...
        with col2:
            ...
            <b>Insight:</b> Program **{program_tercepat}** memiliki proses tercepat, menunjukkan prioritas pada bantuan darurat. Durasi terlama pada program **{program_terlama}** wajar terjadi karena umumnya memerlukan proses verifikasi yang lebih kompleks.
            </div>""", unsafe_allow_html=True)
        ...
        <b>Insight:</b> Grafik ini menunjukkan dinamika setiap program. Garis yang menanjak tajam menandakan pertumbuhan pesat, sementara garis yang landai menunjukkan program yang stabil atau sudah matang.
        </div>""", unsafe_allow_html=True)
...
# ==================================================================
    ...
    # --- 2. HALAMAN DATA PENERIMA BANTUAN ---
    ...
    # --- 3. HALAMAN ANALISIS EKSPLORATIF (EDA) ---
        ...
        with vcol1:
            ...
            avg_top_sub = top_sub.mean()
            fig_v2 = px.bar(top_sub, y=top_sub.index, x=top_sub.values, orientation='h', text_auto=True, labels={'y':'', 'x':'Jumlah Transaksi'})
            fig_v2.add_vline(x=avg_top_sub, line_dash="dash", line_color="red", annotation_text="Rata-rata")
            fig_v2.update_layout(yaxis={'categoryorder':'total ascending'}, dragmode=False)
            st.plotly_chart(fig_v2, use_container_width=True)
            top_program_nama = top_sub.index[0] if not top_sub.empty else "N/A"
            st.markdown(f"<div style='font-size:14px;'><b>Insight:</b> Subprogram **'{top_program_nama}'** adalah aktivitas inti dari program ini.</div>", unsafe_allow_html=True)

        with vcol2:
            st.info("**Top 10 Kota Penerima Bantuan**")
            top_kota = df_filtered_eda['Kota'].value_counts().nlargest(10)
            avg_top_kota = top_kota.mean()
            fig_v5 = px.bar(top_kota, y=top_kota.index, x=top_kota.values, orientation='h', text_auto=True, labels={'y':'', 'x':'Jumlah Transaksi'})
            fig_v5.add_vline(x=avg_top_kota, line_dash="dash", line_color="red", annotation_text="Rata-rata")
            ...
            st.markdown(f"<div style='font-size:14px;'><b>Insight:</b> Wilayah **{top_kota_nama}** menjadi fokus utama penyaluran, menunjukkan basis operasi atau kemitraan yang kuat di area tersebut.</div>", unsafe_allow_html=True)
        
        st.info("**Proporsi Sumber Anggaran**")
        sumber_anggaran = df_filtered_eda['Sumber Anggaran'].value_counts()
        fig_pie = px.pie(sumber_anggaran, names=sumber_anggaran.index, values=sumber_anggaran.values, hole=0.5, title="Proporsi Sumber Anggaran")
        fig_pie.update_traces(textposition='inside', textinfo='percent+label')
        fig_pie.update_layout(dragmode=False)
        st.plotly_chart(fig_pie, use_container_width=True)
        sumber_dominan = sumber_anggaran.index[0] if not sumber_anggaran.empty else "N/A"
        st.markdown(f"<div style='font-size:14px;'><b>Insight:</b> Mayoritas dana bersumber dari **{sumber_dominan}**. Dana Zakat yang proporsinya lebih kecil mungkin dialokasikan untuk subprogram spesifik yang sesuai kriteria mustahik.</div>", unsafe_allow_html=True)

    # --- 4. HALAMAN PROFILING CLUSTER ---
            ...
            if not nama_cluster_options:
                ...
                if selected_cluster_nama:
                    ...
                    if df_cluster.empty:
                        ...
                        with pccol1:
                            st.info("**Perbandingan Durasi Proses antar Cluster**")
                            fig_box_durasi = px.box(df_cluster_all_years, x='Cluster', y='Durasi Total', color='Cluster', labels={'Cluster':'Nomor Cluster'}, points="all")
                            fig_box_durasi.update_layout(dragmode=False)
                            st.plotly_chart(fig_box_durasi, use_container_width=True)
                            st.markdown("""<div style='font-size:14px;'><b>Insight:</b> Grafik ini secara visual membuktikan perbedaan kecepatan proses. Cluster yang dinamai 'Cepat' memiliki sebaran durasi yang konsisten di dekat nol, sementara cluster 'Lambat' menunjukkan sebaran yang jauh lebih panjang.</div>""", unsafe_allow_html=True)
                        with pccol2:
                            st.info("**Perbandingan Jumlah Bantuan antar Cluster**")
                            fig_box_bantuan = px.box(df_cluster_all_years, x='Cluster', y='Jumlah Bantuan', color='Cluster', labels={'Cluster':'Nomor Cluster'})
                            fig_box_bantuan.update_layout(dragmode=False)
                            st.plotly_chart(fig_box_bantuan, use_container_width=True)
                            st.markdown("""<div style='font-size:14px;'><b>Insight:</b> Dari sini terlihat apakah ada cluster yang secara konsisten menerima bantuan lebih besar atau lebih kecil, yang mengvalidasi karakteristik dari setiap segmen.</div>""", unsafe_allow_html=True)
                        
                        st.info("**Peta Persebaran Cluster**")
                        fig_scatter = px.scatter(df_cluster_all_years, x='Durasi Total', y='Jumlah Bantuan', color='Cluster', hover_name='Kat. Subprogram', size='Jumlah Bantuan', size_max=40, labels={'Cluster':'Nomor Cluster'})
                        fig_scatter.update_layout(dragmode=False)
                        st.plotly_chart(fig_scatter, use_container_width=True)
                        st.markdown("""<div style='font-size:14px;'><b>Insight:</b> Peta ini menunjukkan keberhasilan algoritma clustering dalam memisahkan data menjadi kelompok-kelompok berbeda. Gumpalan data yang jelas untuk masing-masing warna (cluster) mengkonfirmasi bahwa segmen yang terbentuk memang memiliki karakteristik yang berbeda secara nyata.</div>""", unsafe_allow_html=True)
